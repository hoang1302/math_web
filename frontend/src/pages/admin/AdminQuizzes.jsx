                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="space-x-2">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        const firstTopic = formData.topics && formData.topics.length > 0 
                          ? topics.find(t => t._id === formData.topics[0])?.title || ''
                          : '';
                        setAiFormData({
                          topic: firstTopic,
                          difficulty: 'medium',
            ...
            {aiGeneratedQuestions.length === 0 ? (
              <div className="space-y-4">
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1">
                    Để trống nếu muốn tạo theo chủ đề của bài thi
                  </p>
                </div>
              ...
              <div>
                <div className="mb-4 flex justify-between items-center">
                  <p className="text-sm text-gray-600">
                    Đã tạo {aiGeneratedQuestions.length} câu hỏi. Chọn câu hỏi muốn thêm vào bài thi:
                  </p>
                  <button
                    onClick={() => {
                      setAiGeneratedQuestions([]);
                  ...
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        // Create exercises and add to quiz
                        const createdExerciseIds = [];
                        ...
                        alert(err.response?.data?.message || 'Có lỗi xảy ra khi thêm câu hỏi');
                      }
                    }}
                    disabled={selectedAiQuestions.length === 0}
                    className="flex-1 px-4 py-2 bg-primary-600 text-white rounded-lg hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Thêm {selectedAiQuestions.length > 0 ? `${selectedAiQuestions.length} ` : ''}câu hỏi đã chọn vào bài thi
                  </button>
                  <button
                    onClick={() => {
                      setShowAIModal(false);
